...
public class SdxUpgradeService {
    ...
    public FlowStartResponse triggerUpgradeByName(String userCrn, String clusterName) {
    ...
    public FlowStartResponse triggerUpgradeByCrn(String userCrn, String clusterCrn) {
    ...
    public void changeImage(Long id, UpgradeOptionV4Response upgradeOption) {
        ...
        if (cluster.isPresent()) {
            ...
            sdxStatusService.setStatusForDatalakeAndNotify(
                    ...
                    "Changing image",
                    cluster.get());
            FlowStartResponse flowStartResponse = stackV4Endpoint.changeImage(0L, cluster.get().getClusterName(), stackImageChangeRequest);
            cloudbreakFlowService.saveLastCloudbreakFlowChainId(cluster.get(), flowStartResponse);
        } else {
    ...
    public void upgrade(Long id) {
        ...
        if (cluster.isPresent()) {
            sdxStatusService.setStatusForDatalakeAndNotify(
                    ...
                    "Upgrade started",
                    cluster.get());
            FlowStartResponse flowStartResponse = stackV4Endpoint.upgradeCluster(0L, cluster.get().getClusterName());
            cloudbreakFlowService.saveLastCloudbreakFlowChainId(cluster.get(), flowStartResponse);
        } else {
            throw new NotFoundException("Not found cluster with id" + id);
        }
